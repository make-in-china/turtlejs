...
namespace ComponentView{
    export class Button{
        orderScript_1=function(this:VPlaceHolder){
    Order.Switch.run({
        condition:'true',
        isBlockStart:Order.Switch.isBlockStart,
        placeholder:this,
        isBreak:false,
        blocks:[
            {
    ...
    `,3).$]
        ...
        orderScript_0=function(this:VPlaceHolder){
    Order.For.run({
        ...
        initDOM(){
            push.call(this.tops,<any>[
                    $$$("div")
.$.$$(this.panel).$
('',20).useThisCall(this.orderScript_0).$
('',20).useThisCall(this.orderScript_1).$
            ]);
